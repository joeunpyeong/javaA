...
public class Ex01_Array {
	...
	//연관이 있는 같은 데이터 타입을 하나로 묶어놓은 그룹이라고 생각하면됨.
	//1차원배열을 구성하는 요소 => 변수들(데이터들)
	
	//2차원 배열:1차원 배열은 x축(가로) y축(세로) 방향으로 행 또는 열만 있는 구조
	//2차원 배열은 1차원 배열을 요소로 가지기때문에 x축과 y축을 동시에 가지고 있는 행열구조
	//[  [1][2][3]
	//	 [4][5][6]
	//]
	
	//규칙 : 차원수=for문 수를 맞춰주면 모든 index(주소)에 접근이 가능하다
	public static void main(String[] args) {
		int[]array1 = new int[3];
		System.out.println(array1[0]);
		int[][]array2= new int[2][3]; //2행 3열(세로 2 ,가로 3)을 가지는 배열을 초기화 시킴
		//배열의 크기 조회.
		System.out.println("1차원 배열의 크기 조회: "+array1.length);
		System.out.println("2차원배열 내부 1차원크기"+array2[0].length);
